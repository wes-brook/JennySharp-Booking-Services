...
function pageTransitionOut() {
  ...
  tl.set(
    ".default-header .col-row, .default-header + .section",
    {
      rotate: 0.001,
      opacity: 0,
      y: "1em",
...
function initBasicFunctions() {
  ...
  $(".col-row-mobile-nav [data-nav-image-id]").on("mouseenter", function () {
    ...
    $("[data-nav-image-group]")
      .find('[data-nav-image-id="' + navImageID + '"]')
      .attr("data-nav-image-status", "active")
      .siblings()
      .attr("data-nav-image-status", "not-active");
  });
  ...
  $("[data-filter-section]").each(function () {
    ...
    filterGroup.find("[data-filter-category]").click(function () {
      ...
      // All Filter
        ...
        filterGroup
          .find('[data-filter-category="' + clickedFilterCategory + '"]')
...
function initMews() {
  ...
  Mews.Distributor(
    {
      configurationIds: ["1802fb4c-34a2-455d-880f-ae4071d5b71a"],
...
// Remove the injected "BOOK NOW" button immediately
const observer = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    mutation.addedNodes.forEach((node) => {
      if (node.nodeType === 1 && node.id === "squire_booking_widget_root") {
        node.remove();
      }
    });
  });
